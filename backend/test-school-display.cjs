  ...
  try {
    ...
    const schoolUsersQuery = `
      SELECT 
        ...
        school_director, school_contact_phone,
        created_at
      FROM users 
      WHERE type = 'school'
    ...
    if (schoolUsers.rows.length > 0) {
      schoolUsers.rows.forEach((user, index) => {
        ...
        console.log(`   Cidade: ${user.school_city || 'N/A'}`);
        console.log(`   Estado: ${user.school_state || 'N/A'}`);
        console.log(`   Diretor: ${user.school_director || 'N/A'}`);
