        ...
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-2 lg:space-x-4">
            {menuItems.map(item => {
              ...
              const isMessagePage = item.path === '/my-messages' || item.path === '/company-messages';
                  ...
                  <Link to={item.path} className={`flex items-center space-x-1 px-2 lg:px-4 py-2 rounded-full transition-all duration-300 font-medium text-sm lg:text-base ${
                    isActive 
                      ? isJourneyPage 
                        ...
                        : isMessagePage
                        ? 'text-green-600 bg-green-50 shadow-sm border-2 border-green-200'
            ...
            {user ? (
              <div className="border-t pt-4 mt-4">
                <div className="px-4 py-2 text-sm text-gray-500 font-medium">
                  Conectado como: {user.name || user.companyName}
                </div>
                <motion.button
                  onClick={handleLogout}
                  className="flex items-center space-x-3 w-full px-4 py-3 text-left text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg transition-colors font-medium"
                  whileTap={{ scale: 0.95 }}
                >
              ...
              <div className="border-t pt-4 mt-4 space-y-2">
                {isOnCompanyPage ? (
                  <Link 
                    to="/" 
                    onClick={() => setIsOpen(false)} 
                    className="flex items-center space-x-3 px-4 py-3 text-gray-600 hover:bg-blue-50 hover:text-blue-600 transition-colors rounded-lg font-medium"
                  >
                ...
                <Link 
                  to="/login" 
                  onClick={() => setIsOpen(false)} 
                  className="flex items-center justify-center px-4 py-3 text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 rounded-lg transition-all font-medium shadow-sm"
                >
                  Entrar
                </Link>
              </div>
