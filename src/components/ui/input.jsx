...
const Input = React.forwardRef(({ className, type, color, ...props }, ref) => {
  ...
  const templateStyles = (() => {
    switch (templateName) {
      case 'professional':
        return 'h-10 rounded-2xl border-slate-200 bg-slate-50/60 text-sm placeholder:text-slate-400 focus:border-blue-400 focus:ring-blue-200';
      case 'minimal':
        return 'h-12 rounded-md border-gray-200 bg-white text-sm placeholder:text-gray-500';
      case 'classic':
        return 'h-12 rounded-xl border-stone-200 bg-stone-50 text-sm placeholder:text-stone-500';
      case 'modern':
        return 'h-12 rounded-2xl border-black/10 bg-white/70 backdrop-blur text-sm placeholder:text-slate-400';
      default:
        return '';
    }
  })();
  ...
  return (
    <input
      type={type}
      className={cn(
        `flex h-12 w-full rounded-xl border-2 bg-white px-4 py-3 text-sm ring-offset-white file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200 ${finalColorClasses}`,
        templateStyles,
        className
      )}
      ref={ref}
