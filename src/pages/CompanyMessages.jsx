...
const CompanyMessages = () => {
  ...
  const loadConversations = async () => {
    try {
      ...
      console.log('🔍 Frontend (empresa) - Conversas carregadas:', response);
  ...
  // Função para abrir WhatsApp para candidato
  const openWhatsApp = (conversation) => {
    console.log('🔍 WhatsApp (empresa) - Conversa:', conversation);
    if (!conversation.contactPhone) {
      console.log('❌ WhatsApp - Telefone não encontrado');
      toast({
    ...
    const message = `Olá ${conversation.contactName || ''}! Gostaríamos de conversar sobre sua candidatura.`;
                          ...
                          >
                            <div className="flex items-center space-x-3">
                              <div className="relative">
                                ...
                                {/* Badge da escola */}
                                {(conversation.schoolName || conversation.schoolProfileImage) && (
                                  <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full overflow-hidden bg-white flex items-center justify-center shadow-md ring-1 ring-white">
                                    {conversation.schoolProfileImage ? (
                                      <img src={conversation.schoolProfileImage} alt={conversation.schoolName || 'Escola'} className="w-full h-full object-contain" />
                                    ) : (
                  ...
                  {selectedConversation ? (
                      ...
                      <div className="h-[72px] px-5 bg-white border-b border-gray-200 flex items-center">
                        <div className="flex items-center justify-between w-full">
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="relative">
                              <div className="w-10 h-10 rounded-full flex items-center justify-center overflow-hidden bg-gradient-to-br from-[var(--color1)] to-[var(--color2)]">
                                {selectedConversation.contactProfileImage ? (
                                  <img src={selectedConversation.contactProfileImage} alt={selectedConversation.contactName} className="w-full h-full object-cover" />
                                ) : (
                                  <User className="w-5 h-5 text-white" />
                                )}
                              </div>
                              {/* Badge da escola */}
                              {(selectedConversation.schoolName || selectedConversation.schoolProfileImage) && (
                                <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full overflow-hidden bg-white flex items-center justify-center shadow-md ring-1 ring-white">
                                  {selectedConversation.schoolProfileImage ? (
                                    <img src={selectedConversation.schoolProfileImage} alt={selectedConversation.schoolName || 'Escola'} className="w-full h-full object-contain" />
                                  ) : (
                                    <div className="w-full h-full bg-emerald-100 flex items-center justify-center">
                                ...
                                >
                                  {(() => {
                                    ...
                                    const bubbleClass = onlyUrl
                                      ...
                                      : (message.isFromMe ? 'bg-gradient-to-r from-[var(--color1)] to-[var(--color2)] text-white' : 'bg-white text-gray-900 shadow-sm border border-gray-100');
                                    const timeClass = onlyUrl ? 'text-gray-400' : (message.isFromMe ? 'text-blue-100' : 'text-gray-500');
                                    return (
                      ...
                      <div className="bg-white border-t border-gray-200 p-6">
                        <div className="flex items-center space-x-3">
                          <Input
                            ...
                            className="flex-1 rounded-xl border-2 border-gray-200 focus:border-[var(--color1)] px-4 py-3"
                            onKeyPress={(e) => {
                          ...
                          <Button
                            ...
                            className="bg-gradient-to-r from-[var(--color1)] to-[var(--color2)] hover:from-[var(--color2)] hover:to-[var(--color3)] text-white px-6 py-3 rounded-xl transition-all duration-200"
                          >
                            {sending ? (
                    ...
                    <div className="flex-1 flex items-center justify-center bg-gray-50">
                      <div className="text-center">
                        ...
                        <p className="text-gray-600 max-w-md mx-auto">Escolha um candidato na lista à esquerda para começar a conversar ou continuar uma conversa existente</p>
                      </div>
                    </div>
                  )}
                </div>
