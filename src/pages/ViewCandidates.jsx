                        ...
                        <div className="px-4 sm:px-5 pb-3 sm:pb-4">
                          <div className="flex gap-2">
                            <Button
                              variant="outline"
                              size="sm"
                              disabled={addedContacts.has(candidate.candidate_id)}
                              className={`flex-1 rounded-xl transition-all text-xs sm:text-sm h-8 sm:h-9 ${
                                addedContacts.has(candidate.candidate_id)
                                  ? 'bg-blue-600 border-blue-600 text-white cursor-default'
                                  : 'border-blue-200 text-blue-700 hover:bg-blue-50'
                              }`}
                              onClick={() => {
                                if (addedContacts.has(candidate.candidate_id)) return;
                                chatAPI.saveCandidate(candidate.candidate_id)
                                  .then(() => {
                                    setAddedContacts(prev => new Set([...prev, candidate.candidate_id]));
                                    toast({ title: 'Candidato adicionado', description: 'Acesse Mensagens para conversar.' });
                                  })
                                  .catch(() => toast({ title: 'Erro', description: 'Não foi possível adicionar.', variant: 'destructive' }));
                              }}
                            >
                              {addedContacts.has(candidate.candidate_id) ? (
                                <>
                            ...
                            <Button
                              variant="outline"
                              size="sm"
                              className="flex-1 rounded-xl border-green-200 text-green-600 hover:bg-green-500 hover:text-white hover:border-green-500 transition-all"
